    ...
    return function SNSClient(req, res) {
        ...
        req.on('end', function() {
            ...
            try {
                ...
                return cb(new Error('Error parsing JSON'), null, req, res);
            }
            validateRequest(opts, message, function(err){
                if(err) {
                    return cb(err, null, req, res);
                }
                ...
                if (message.Type === 'SubscriptionConfirmation') {
                    return cb(null,message.SubscribeURL, req, res);
                }
                if(message.Type === 'Notification') {
                    return cb(null, message, req, res);
                }
            });
        });
        // end the request by default
        if (opts.preserveRes !== true) {
            res.end();
        }
    };
}

SNSClient.validateRequest = validateRequest;
